...
:::note
The following guide assumes that you are already familiar with React. <br/>
Please refer to [@rbxts/react](https://github.com/littensy/rbxts-react) and [@rbxts/react-roblox] for more information.
:::

## Introduction

While roblox-ts allows you to use React just like you would in Luau, it also supports a "JSX" shorthand form.
...
If an element is given the `Key` attribute and it **not** a child of another element, it will be wrapped in a `React.Fragment`.
...
The `Ref` attribute directly maps to the `[React.Ref]` key in Luau.
...
const listItems = new Array<React.Element>();
for (let i = 0; i < 10; i++) {
...
To create a Fragment with JSX you can either use the tag name `React.Fragment`
